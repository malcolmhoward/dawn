...
(function() {
      ...
      switch (msg.type) {
        ...
        case 'context':
          ...
          if (activeConvId && msg.payload.current && msg.payload.max) {
            DawnHistory.requestUpdateContext(activeConvId, msg.payload.current, msg.payload.max);
          }
          break;
  ...
  async function init() {
    ...
    DawnAudioCapture.setCallbacks({
      ...
      onError: function(msg) { DawnTranscript.addEntry('system', msg); },
      getOpusEncoder: function() { return { ready: opusReady, worker: opusWorker }; }
    });
