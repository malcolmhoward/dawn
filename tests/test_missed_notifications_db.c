...
static void test_insert_basic(void) {
   ...
   TEST_ASSERT(strcmp(rows[0].status, "ringing") == 0, "status round-trips");
   TEST_ASSERT(strcmp(rows[0].name, "Egg") == 0, "name round-trips");
   TEST_ASSERT(strcmp(rows[0].message, "Timer done") == 0, "message round-trips");
   TEST_ASSERT(rows[0].fire_at == now, "fire_at round-trips");
   TEST_ASSERT(rows[0].conversation_id == 0, "conversation_id defaults to 0");
}
...
static void test_insert_rejects_bad_args(void) {
   ...
   time_t now = time(NULL);
   TEST_ASSERT(missed_notif_insert(0, 1, "timer", "ringing", "n", "m", now, 0) == AUTH_DB_FAILURE,
               "user_id 0 rejected");
   TEST_ASSERT(missed_notif_insert(-1, 1, "timer", "ringing", "n", "m", now, 0) == AUTH_DB_FAILURE,
               "negative user_id rejected");
   TEST_ASSERT(missed_notif_insert(1, 1, NULL, "ringing", "n", "m", now, 0) == AUTH_DB_FAILURE,
               "NULL event_type rejected");
   TEST_ASSERT(missed_notif_insert(1, 1, "timer", NULL, "n", "m", now, 0) == AUTH_DB_FAILURE,
               "NULL status rejected");
   TEST_ASSERT(missed_notif_insert(1, 1, "timer", "ringing", NULL, "m", now, 0) == AUTH_DB_FAILURE,
               "NULL name rejected");

   /* NULL message is allowed (stored as empty string). */
   int rc = missed_notif_insert(1, 1, "timer", "ringing", "n", NULL, now, 0);
   TEST_ASSERT(rc == AUTH_DB_SUCCESS, "NULL message accepted (treated as empty)");
}
...
static void test_insert_cap_enforced(void) {
   ...
   TEST_ASSERT(ok == MISSED_NOTIF_MAX_PER_USER, "first N inserts succeed (where N == cap)");

   /* N+1 must be rejected. */
   int rc = missed_notif_insert(1, 999, "timer", "ringing", "n", "m", now, 0);
   TEST_ASSERT(rc == AUTH_DB_FAILURE, "insert past cap rejected");
   ...
   TEST_ASSERT(stored == MISSED_NOTIF_MAX_PER_USER, "row count stays at cap after rejection");

   /* Another user is unaffected by user 1's cap. */
   int rc2 = missed_notif_insert(2, 1, "timer", "ringing", "n", "m", now, 0);
   TEST_ASSERT(rc2 == AUTH_DB_SUCCESS, "cap is per-user, other user can still insert");
}
...
static void test_get_ordering_oldest_first(void) {
   ...
   TEST_ASSERT(rows[2].event_id == 30, "newest last");
}
...
static void test_delete_by_user_rejects_bad_args(void) {
   ...
   TEST_ASSERT(missed_notif_delete_by_user(0, 1) == AUTH_DB_FAILURE, "id 0 rejected");
   TEST_ASSERT(missed_notif_delete_by_user(-1, 1) == AUTH_DB_FAILURE, "negative id rejected");
   TEST_ASSERT(missed_notif_delete_by_user(1, 0) == AUTH_DB_FAILURE, "user_id 0 rejected");
}
...
int main(void) {
   printf("=== Missed Notifications DB Unit Tests ===\n");
