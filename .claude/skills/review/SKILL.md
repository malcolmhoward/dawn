...
- If an agent flags an issue in code the diff touches, don't dismiss it just because the issue existed before the diff ("pre-existing"). If it's a real, consequential issue — a real bug, a real security concern, a real resource problem — it matters regardless of when it was introduced. The review found it, and ignoring it creates tech debt. Triage these on their own merits (severity, fix complexity, risk) just like any other finding. Only skip pre-existing findings that are genuinely inconsequential (nitpicks, style preferences) or would require invasive changes far outside the diff scope.
