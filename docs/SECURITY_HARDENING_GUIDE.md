...
| **No 2FA** | Password-only authentication. One compromised password = full access to your assistant and connected smart home devices. | TOTP support is designed (see `docs/TODO.md` §6) but not yet implemented. |
...
| **Shared PSK for satellites** | Per-device certs (mTLS) add significant setup complexity for minimal gain on a home LAN. PSK + TLS is appropriate for the threat model. See `docs/TODO.md` Deferred section. |
...
| **No 2FA** | Planned but not yet implemented. See `docs/TODO.md` §6. |
...
The current audit logging covers the security-critical events: authentication, authorization failures, and rate limiting. These are the events an admin needs to detect attacks or investigate incidents. Tool execution and config change logging are observability improvements — valuable but not security-blocking. The audit log is accessible via `dawn-admin log show` (see `docs/FAQ.md`).
