...
ring_buffer_t *ring_buffer_create(size_t capacity) {
   ...
   if (!rb->buffer) {
      LOG_ERROR("Failed to allocate ring buffer storage of %zu bytes", capacity);
      free(rb);
      return NULL;
   }
   ...
   if (pthread_mutex_init(&rb->mutex, NULL) != 0) {
      LOG_ERROR("Failed to initialize ring buffer mutex");
      free(rb->buffer);
      free(rb);
      return NULL;
   }
   ...
   if (pthread_cond_init(&rb->cond, NULL) != 0) {
      LOG_ERROR("Failed to initialize ring buffer condition variable");
   ...
   LOG_INFO("Ring buffer created: capacity=%zu bytes (%.1f seconds at 16kHz mono)", capacity,
