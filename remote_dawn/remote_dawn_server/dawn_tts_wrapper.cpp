...
int dawn_tts_init(const char *model_path) {
   ...
   try {
      ...
      // Load voice model
      piper::loadVoice(tts_config, modelPath, configPath, tts_voice, speakerIdOpt, false);

      // Log final configuration
      LOG_INFO("TTS Configuration:");
      LOG_INFO("  Sample rate: %d Hz", tts_voice.synthesisConfig.sampleRate);
      LOG_INFO("  Sample width: %d bytes (%d-bit)", tts_voice.synthesisConfig.sampleWidth,
...
int dawn_generate_tts_wav(const char *text, uint8_t **wav_data_out, size_t *wav_size_out) {
   ...
   try {
      ...
      piper::textToWavFile(tts_config, tts_voice, std::string(text), audioStream, result);
