...
int verify_tts_wav_format(const uint8_t *wav_data, size_t wav_size) {
   ...
   LOG_INFO("TTS WAV format: %uHz, %u channels, %u-bit, format %u", sample_rate, num_channels,
...
char *process_audio_with_vosk(VoskProcessor *processor, const PCMData *pcm) {
   ...
   LOG_INFO("Processing audio with Vosk: %zu bytes, %.2f seconds", pcm->pcm_size,
            pcm->duration_seconds);

   // Verify audio format compatibility
   if (pcm->sample_rate != VOSK_SAMPLE_RATE) {
      LOG_WARNING("Sample rate mismatch - PCM: %u Hz, Vosk expects: %d Hz", pcm->sample_rate,
                  VOSK_SAMPLE_RATE);
   }

   if (pcm->bits_per_sample != 16) {
      LOG_WARNING("Bit depth mismatch - PCM: %u bits, Vosk expects: 16 bits", pcm->bits_per_sample);
   }

   if (pcm->num_channels != 1) {
      LOG_WARNING("Channel mismatch - PCM: %u channels, Vosk expects: 1 (mono)", pcm->num_channels);
   }
   ...
   if (json_object_object_get_ex(json_result, "text", &text_obj)) {
      ...
      if (text && strlen(text) > 0) {
         ...
         LOG_INFO("Transcription successful: \"%s\" (confidence: %.2f)", text,
...
PCMData *extract_pcm_from_wav(const uint8_t *wav_data, size_t wav_size) {
   ...
   if (expected_data_end > wav_size) {
      LOG_WARNING("Data size mismatch - header says %u bytes, but only %zu available", data_bytes,
   ...
   if (pcm->is_valid) {
      LOG_INFO("PCM extraction successful: %zu samples, %.2f seconds", pcm->num_samples,
               pcm->duration_seconds);
   } else {
         ...
         if (pcm) {
            ...
            if (transcription && strlen(transcription) > 0) {
               ...
               if (llm_response && strlen(llm_response) > 0) {
                  ...
                  // TTS Generation
                  int tts_result = dawn_generate_tts_wav(llm_response, &tts_wav_data,
                                                         &tts_wav_size);
                  ...
                  if (tts_result == 0 && tts_wav_data && tts_wav_size > 0) {
                     ...
                     if (verify_tts_wav_format(tts_wav_data, tts_wav_size)) {
                        if (check_response_size_limit(tts_wav_size)) {
                           ...
                           if (truncate_wav_response(tts_wav_data, tts_wav_size, &truncated_data,
                                                     &truncated_size) == 0) {
                              free(tts_wav_data);
                        ...
                        uint8_t *error_tts_data = generate_error_tts(ERROR_MSG_TTS_FAILED,
                                                                     &tts_wav_size);
...
processing_done:
                  ...
                  size_t tts_wav_size = 0;
                  uint8_t *error_tts_data = generate_error_tts(ERROR_MSG_LLM_TIMEOUT,
                                                               &tts_wav_size);
